...
def to_console(r: ScanResult) -> str:
    ...
    for f in r.findings[:100]:
        ...
        lines.append(
            f"  {ICON[f.severity]} [{sev:<10}] {f.id:<14} {f.title}"
        )
        if f.location:
...
def to_sarif(r: ScanResult) -> str:
    ...
    def _uri(loc: str) -> str:
        return loc.split(":")[0] if loc else "unknown"
            ...
            "runs": [
                {
                    ...
                    "properties": {
                        "classification": r.classification_placeholder
                    },
                    "results": [
                        {
                            "ruleId": f.id,
